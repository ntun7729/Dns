...
class FrpcSupervisor:
    ...
    def __init__(self, settings: Settings, runtime: RuntimeState, config: Path) -> None:
    ...
    def _mark_reconnecting(self, error: str | None = None) -> None:
        ...
        self.runtime.update(frpc_state="reconnecting", frpc_running=False, frpc_last_error=error)
        ...
        try:
            for line in stream:
                ...
                if "try to connect to server" in lower:
                    ...
                    self.runtime.update(frpc_state="connected", frpc_running=False, frpc_last_error=None)
                    ...
                    self.runtime.update(frpc_last_error=safe, frpc_running=False)
    ...
    def _wait_for_dot(self) -> bool:
        while not self.stop_event.is_set():
            if not self.settings.dot_enabled:
                return True
            if self.runtime.snapshot().get("dot_state") == "running":
                return True
            self.runtime.update(
                frpc_state="blocked",
                frpc_running=False,
    ...
    def _run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            self.runtime.update(
                ...
                frpc_last_error=f"FRPC exited with code {code}; reconnecting in {delay:.1f} seconds.",
                frpc_next_retry_seconds=delay,
            )
            if self.stop_event.wait(delay):
                break
        self.runtime.update(
            frpc_running=False,
            frpc_next_retry_seconds=0.0,
            frpc_state="disabled" if not self.settings.frpc_enabled else "stopped",
        )


def start_frpc(settings: Settings, runtime: RuntimeState) -> FrpcSupervisor | None:
