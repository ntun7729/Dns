...
def upstream_overall_state(upstreams: Sequence[Mapping[str, Any]]) -> str:
    ...
    if states == {"unknown"} or not states:
        return "unknown"
...
def diagnostics(
    ...
    if frpc_state not in {"running", "disabled"}:
        reconnects = int(payload.get("frpc", {}).get("reconnect_count") or 0)
        next_retry = float(payload.get("frpc", {}).get("next_retry_seconds") or 0)
        if frpc_state == "reconnecting":
            detail = (
                f" Retry scheduled in about {next_retry:.1f}s."
                if next_retry > 0
                else " FRPC is retrying now."
            )
            message = f"FRPC is reconnecting ({reconnects} reconnect events).{detail}"
            severity = "warning"
        elif frpc_state == "connected":
            message = "FRPC reached FRPS and is waiting for the DoT proxy to become active."
            severity = "warning"
        else:
            message = f"FRPC state is {frpc_state}."
            severity = "critical" if frpc_state in {"exited", "startup-failed", "stopped"} else "warning"
        findings.append(
            {
                "severity": severity,
                "code": "frpc_state",
        ...
        findings.append(
            {
                ...
                "message": "At least one upstream resolver is cooling down; failover remains available.",
            }
        )
    if payload["metrics"]["dns_errors"]:
        findings.append(
            {
                "severity": "warning",
                "code": "dns_errors",
                "message": f"{payload['metrics']['dns_errors']} DNS processing errors have been recorded.",
            }
        )
        ...
        findings.append(
            {
                ...
                "message": "Dashboard controls are disabled until credentials are configured.",
            }
        )
    if filter_status.get("last_error"):
        findings.append(
            {
                "severity": "warning",
                "code": "blocklist_update_failed",
                "message": "A blocklist update failed; the previous in-memory list remains active.",
            }
        )
    if not findings:
        findings.append(
...
def status_payload(
    ...
    snapshot = runtime.snapshot()
    readiness = readiness_payload(settings, runtime)
    profile = profiles.active()
    filtering = blocklists.status(profile)
    upstreams = runtime.public_upstream_stats(profile.upstreams)
    upstream_state = upstream_overall_state(upstreams)
    samples = int(snapshot["upstream_latency_samples"])
    total = float(snapshot["upstream_latency_total_ms"])
    certificate = dict(snapshot["certificate"])
    certificate["warning"] = certificate_warning(certificate)
    public_endpoint = (
    ...
    payload: dict[str, Any] = {
        ...
        "endpoints": {
            ...
            "upstream_resolver": f"{len(profile.upstreams)} configured resolvers",
        },
        "frpc": {
            "state": snapshot["frpc_state"],
            "authentication": settings.frp_auth_mode,
            "started_at": snapshot.get("frpc_started_at"),
            "connected_at": snapshot.get("frpc_connected_at"),
            "last_exit_at": snapshot.get("frpc_last_exit_at"),
            "exit_code": snapshot.get("frpc_exit_code"),
            "last_error": snapshot.get("frpc_last_error"),
            "reconnect_count": int(snapshot.get("frpc_reconnect_count", 0) or 0),
            "next_retry_seconds": float(snapshot.get("frpc_next_retry_seconds", 0.0) or 0.0),
            "session_seconds": (
                seconds_since(snapshot.get("frpc_connected_at") or snapshot.get("frpc_started_at"))
                if snapshot["frpc_state"] == "running"
                else None
            ),
        },
        "metrics": {
            ...
            "dns_error_types": snapshot["dns_error_types"],
            "client_disconnects": snapshot["unexpected_disconnects"],
            "active_connections": snapshot["active_connections"],
            "peak_connections": snapshot["peak_connections"],
            "last_query_at": snapshot["last_query_at"],
            "upstream_last_used": snapshot["upstream_last_used"],
            "upstream_failovers": snapshot["upstream_failovers"],
            "upstream_last_latency_ms": snapshot["upstream_last_latency_ms"],
            "upstream_average_latency_ms": round(total / samples, 2) if samples else None,
            "upstream_samples": samples,
            "upstream_last_success_at": snapshot["upstream_last_success_at"],
            "upstream_last_failure_at": snapshot["upstream_last_failure_at"],
            "upstreams": upstreams,
        },
        "history": runtime.history.snapshot(),
        "filtering": filtering,
        "profile": {
            "id": profile.id,
            "name": profile.name,
        ...
        "access_control": {
            ...
            "controls_available": settings.auth_enabled,
        },
        "configuration": {
            **settings.public_config(),
            "upstream_servers": [endpoint.key for endpoint in profile.upstreams],
            "upstream_strategy": profile.strategy,
        },
    }
    payload["diagnostics"] = diagnostics(payload, filtering, settings)
    return payload
